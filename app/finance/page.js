...
export default function FinancePage() {
  ...
  const statusLabel = (status) => ({
    draft: "草稿", confirmed: "已確認", shipped: "已出貨", completed: "已完成"
  }[status] || status)
    ...
    <div className="min-h-screen bg-gray-100">
      <header className="bg-white border-b border-gray-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-base text-gray-400">商品與財務端 / 應收帳款對帳單</p>
          </div>
        </div>
      </header>
      <main className="max-w-7xl mx-auto px-6 py-6 space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-white rounded-2xl p-5 shadow-sm">
            <p className="text-base text-gray-500">合計應收金額</p>
            <p className="text-3xl font-bold text-blue-700 mt-1">${totalAmount.toLocaleString()}</p>
          </div>
          <div className="bg-white rounded-2xl p-5 shadow-sm">
            <p className="text-base text-gray-500">已完成金額</p>
            <p className="text-3xl font-bold text-green-600 mt-1">
              ${data.filter(r => r.status === "completed").reduce((s, r) => s + Number(r.total || 0), 0).toLocaleString()}
            </p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100">
            <h2 className="text-xl font-bold text-gray-700">應收帳款對帳單</h2>
          </div>
          ...
          : (
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>{["銷貨單號", "客戶名稱", "銷貨日期", "出貨日期", "狀態", "含稅金額"].map(h => (
              ...
              <tbody className="divide-y divide-gray-50">
                {data.map(row => (
                  <tr key={row.id} className="hover:bg-blue-50/30">
                    ...
                    <td className="px-5 py-4 text-base text-gray-500">{row.delivery_date ? new Date(row.delivery_date).toLocaleDateString("zh-TW") : "—"}</td>
                    <td className="px-5 py-4"><span className={`px-2.5 py-0.5 rounded-full text-sm font-medium ${statusColor(row.status)}`}>{statusLabel(row.status)}</span></td>
                    <td className="px-5 py-4 text-lg font-bold text-blue-700">${Number(row.total || 0).toLocaleString()}</td>
                  </tr>
                ))}
                <tr className="bg-blue-50">
                  <td colSpan={5} className="px-5 py-4 text-base font-bold text-right text-gray-700">合計</td>
