...
import { ChevronLeft, Save, RotateCcw, Plus, Trash2, Search } from "lucide-react"
...
function genNo() {
  ...
  return `SR${ymd}${String(Math.floor(Math.random() * 900) + 100)}`
}
...
export default function SalesReturnForm() {
  ...
  const [form, setForm] = useState({
    ...
    original_order_no: "", reason: "", status: "draft",
    tax_type: "taxed", subtotal: 0, tax_amount: 0, total: 0, notes: ""
  })
  ...
  useEffect(() => {
    ...
    if (!isNew) {
      fetch(`/api/sales-return/${id}`).then(r => r.json()).then(data => {
  ...
  const calcTotals = useCallback((itemList) => {
  ...
  const updateItem = (index, field, value) => {
    setItems(prev => {
      ...
      setForm(f => ({ ...f, ...calcTotals(updated) }))
      return updated
    })
  }

  const addItem = () => setItems(prev => [...prev, { ...EMPTY_ITEM }])
  const removeItem = (i) => setItems(prev => { const u = prev.filter((_, idx) => idx !== i); setForm(f => ({ ...f, ...calcTotals(u) })); return u })

  const pickCustomer = (c) => { setForm(f => ({ ...f, customer_name: c.short_name })); setShowCustomerList(false) }
  const pickProduct = (product, index) => {
    setItems(prev => {
      const updated = prev.map((item, i) => {
        if (i !== index) return item
        const qty = Number(item.quantity) || 1; const price = Number(product.retail_price) || 0
  ...
  const handleSave = async () => {
    ...
    const payload = { ...form, items: validItems }; delete payload.sales_return_items
    const res = await fetch(isNew ? "/api/sales-return" : `/api/sales-return/${id}`,
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <header className="bg-white border-b border-gray-200 shadow-sm">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link href="/sales-return" className="text-gray-400 hover:text-gray-600"><ChevronLeft size={24} /></Link>
            <div className="w-10 h-10 bg-orange-500 rounded-xl flex items-center justify-center">
              <RotateCcw className="w-6 h-6 text-white" />
            </div>
            <div>
              <h1 className="text-2xl font-bold text-gray-800">{isNew ? "新增銷貨退回單" : "編輯銷貨退回單"}</h1>
              <p className="text-base text-gray-400">客戶端 / 退貨作業</p>
            </div>
          </div>
          <div className="flex gap-3">
            <Link href="/sales-return" className="px-5 py-2.5 border-2 border-gray-200 text-lg rounded-xl hover:bg-gray-50">取消</Link>
      ...
      <main className="max-w-6xl mx-auto px-6 py-6 space-y-6">
        ...
        <section className="bg-white rounded-2xl p-6 shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            <div><label className="block text-base font-semibold text-gray-600 mb-1">退回單號</label>
              <input value={form.return_no} onChange={e => setForm(f => ({ ...f, return_no: e.target.value }))} className={inputCls} /></div>
            <div className="relative"><label className="block text-base font-semibold text-gray-600 mb-1"><span className="text-red-500 mr-1">*</span>客戶名稱</label>
              <input value={form.customer_name}
                onChange={e => { setForm(f => ({ ...f, customer_name: e.target.value })); setCustomerQ(e.target.value); setShowCustomerList(true) }}
                onFocus={() => setShowCustomerList(true)} className={inputCls} placeholder="輸入客戶名稱" />
              {showCustomerList && filteredCustomers.length > 0 && (
                <div className="absolute z-20 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg max-h-48 overflow-y-auto">
                  {filteredCustomers.map(c => (
                    <button key={c.id} type="button" onClick={() => pickCustomer(c)} className="w-full px-4 py-2.5 text-left text-base hover:bg-orange-50 flex gap-3">
                      <span className="font-mono text-gray-400 text-sm">{c.code}</span><span className="font-semibold">{c.short_name}</span>
                    </button>
                  ))}
                </div>
              )}</div>
            <div><label className="block text-base font-semibold text-gray-600 mb-1">退回日期</label>
              <input type="date" value={form.return_date} onChange={e => setForm(f => ({ ...f, return_date: e.target.value }))} className={inputCls} /></div>
            <div><label className="block text-base font-semibold text-gray-600 mb-1">原銷貨單號</label>
              <input value={form.original_order_no || ""} onChange={e => setForm(f => ({ ...f, original_order_no: e.target.value }))} placeholder="選填" className={inputCls} /></div>
            <div className="md:col-span-2"><label className="block text-base font-semibold text-gray-600 mb-1">退回原因</label>
        ...
        <section className="bg-white rounded-2xl p-6 shadow-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {items.map((item, idx) => (
                  <tr key={idx}>
                            ...
                            <div className="max-h-40 overflow-y-auto">
                              {filteredProducts.map(p => (
                                <button key={p.id} type="button" onClick={() => pickProduct(p, idx)}
                                  ...
                                  <span className="text-orange-600">${p.retail_price}</span>
                                </button>
                              ))}
                            </div>
                    ...
                    <td className="px-3 py-2"><input value={item.product_name} onChange={e => updateItem(idx, "product_name", e.target.value)}
                      className="w-full px-2 py-1.5 border border-gray-200 rounded-lg focus:outline-none" placeholder="品名" /></td>
                    <td className="px-3 py-2 w-16"><input value={item.unit} onChange={e => updateItem(idx, "unit", e.target.value)}
                      className="w-full px-2 py-1.5 border border-gray-200 rounded-lg focus:outline-none" /></td>
                    <td className="px-3 py-2 w-24"><input type="number" min={0} value={item.quantity} onChange={e => updateItem(idx, "quantity", e.target.value)}
                      className="w-full px-2 py-1.5 text-right border border-gray-200 rounded-lg focus:outline-none" /></td>
                    <td className="px-3 py-2 w-28"><input type="number" min={0} value={item.unit_price} onChange={e => updateItem(idx, "unit_price", e.target.value)}
                      className="w-full px-2 py-1.5 text-right border border-gray-200 rounded-lg focus:outline-none" /></td>
          ...
          <div className="mt-4 flex justify-end">
            <div className="w-56 space-y-1.5 text-base">
              ...
              <div className="flex justify-between text-xl font-bold pt-2 border-t border-gray-200"><span>總計</span><span className="text-orange-600">${Number(form.total).toLocaleString()}</span></div>
            </div>
          </div>
        </section>
        <section className="bg-white rounded-2xl p-6 shadow-sm">
        ...
        <div className="flex justify-end gap-3 pb-8">
          <Link href="/sales-return" className="px-8 py-3 border-2 border-gray-200 text-lg rounded-xl hover:bg-gray-50">取消</Link>
          <button onClick={handleSave} disabled={saving}
      ...
      {(showCustomerList || showProductPicker !== null) && <div className="fixed inset-0 z-10" onClick={() => { setShowCustomerList(false); setShowProductPicker(null) }} />}
    </div>
  )
}
