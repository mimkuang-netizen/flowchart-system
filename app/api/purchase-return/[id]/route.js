...
export async function GET(request, { params }) {
  ...
  const { data, error } = await supabase.from('purchase_returns').select('*, purchase_return_items(*)').eq('id', id).single()
...
export async function PUT(request, { params }) {
  ...
  delete header.id; delete header.created_at; delete header.purchase_return_items
  const { data, error } = await supabase.from('purchase_returns').update(header).eq('id', id).select().single()
