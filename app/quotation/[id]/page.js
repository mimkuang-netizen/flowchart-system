...
export default function QuotationForm() {
  ...
  const openQuickAdd = () => {
    ...
    setQuickAddError(""); setShowQuickAdd(true); setShowCustomerList(false)
  }
  const submitQuickAdd = async () => {
    ...
    try {
      ...
      pickCustomer(data); setShowQuickAdd(false)
                ...
                {showCustomerList && (filteredCustomers.length > 0 || customerQ) && (
                  <div className="absolute z-20 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg max-h-60 overflow-y-auto">
                    ...
                    <button type="button" onClick={openQuickAdd}
                      className="w-full px-4 py-3 text-left text-base hover:bg-green-50 text-green-700 font-semibold flex items-center gap-2">
                      <span className="text-lg">＋</span>新增客戶{customerQ ? ` 「${customerQ}」` : ""}
                    </button>
                  </div>
                )}
              </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input value={quickAddData.short_name} onChange={e => setQuickAddData(d => ({ ...d, short_name: e.target.value }))} className="w-full px-3 py-2 text-lg border border-gray-300 rounded-xl focus:outline-none focus:border-orange-400" />
              </div>
              <div>
                <label className="block text-base font-semibold text-gray-600 mb-1">客戶代號（選填）</label>
                <input value={quickAddData.code} onChange={e => setQuickAddData(d => ({ ...d, code: e.target.value }))} className="w-full px-3 py-2 text-lg border border-gray-300 rounded-xl focus:outline-none focus:border-orange-400" />
              </div>
              <div>
                <label className="block text-base font-semibold text-gray-600 mb-1">公司全名（選填）</label>
                <input value={quickAddData.full_name} onChange={e => setQuickAddData(d => ({ ...d, full_name: e.target.value }))} className="w-full px-3 py-2 text-lg border border-gray-300 rounded-xl focus:outline-none focus:border-orange-400" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-base font-semibold text-gray-600 mb-1">電話（選填）</label>
                  <input value={quickAddData.phone} onChange={e => setQuickAddData(d => ({ ...d, phone: e.target.value }))} className="w-full px-3 py-2 text-lg border border-gray-300 rounded-xl focus:outline-none focus:border-orange-400" />
                </div>
                <div>
                  <label className="block text-base font-semibold text-gray-600 mb-1">聯絡人（選填）</label>
                  <input value={quickAddData.contact} onChange={e => setQuickAddData(d => ({ ...d, contact: e.target.value }))} className="w-full px-3 py-2 text-lg border border-gray-300 rounded-xl focus:outline-none focus:border-orange-400" />
                </div>
              </div>
            ...
            <div className="flex justify-end gap-3 mt-5">
              ...
              <button onClick={submitQuickAdd} disabled={quickAddSaving} className="px-5 py-2.5 bg-orange-500 text-white text-base font-semibold rounded-xl hover:bg-orange-600 disabled:opacity-50">
