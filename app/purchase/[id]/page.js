...
export default function PurchaseForm() {
  ...
  useEffect(() => {
    ...
    if (!isNew) {
      fetch(`/api/purchase/${id}`).then(r => r.json()).then(data => {
        const { purchase_order_items, ...header } = data
        setForm(header)
        setItems(purchase_order_items?.length > 0 ? purchase_order_items.sort((a, b) => a.sort_order - b.sort_order) : [{ ...EMPTY_ITEM }])
        setLoading(false)
      }).catch(() => { setError("找不到此採購單"); setLoading(false) })
    }
  }, [id, isNew])
  ...
  const handleSave = async () => {
    ...
    const payload = { ...form, items: validItems }; delete payload.purchase_order_items
    const res = await fetch(isNew ? "/api/purchase" : `/api/purchase/${id}`,
      ...
      <header className="bg-white border-b border-gray-200 shadow-sm">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link href="/purchase" className="text-gray-400 hover:text-gray-600"><ChevronLeft size={24} /></Link>
            <div className="w-10 h-10 bg-green-600 rounded-xl flex items-center justify-center"><ClipboardList className="w-6 h-6 text-white" /></div>
            <div>
              <h1 className="text-2xl font-bold text-gray-800">{isNew ? "新增採購單" : "編輯採購單"}</h1>
      ...
      <main className="max-w-6xl mx-auto px-6 py-6 space-y-6">
        ...
        <section className="bg-white rounded-2xl p-6 shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            <div><label className="block text-base font-semibold text-gray-600 mb-1">採購單號</label>
              <input value={form.po_no} onChange={e => setForm(f => ({ ...f, po_no: e.target.value }))} className={inputCls} /></div>
            <div className="relative"><label className="block text-base font-semibold text-gray-600 mb-1"><span className="text-red-500 mr-1">*</span>廠商名稱</label>
              <input value={form.vendor_name}
                ...
                onFocus={() => setShowVendorList(true)} className={inputCls} placeholder="輸入廠商名稱或代號" />
            ...
            <div><label className="block text-base font-semibold text-gray-600 mb-1">採購日期</label>
              <input type="date" value={form.po_date} onChange={e => setForm(f => ({ ...f, po_date: e.target.value }))} className={inputCls} /></div>
            <div><label className="block text-base font-semibold text-gray-600 mb-1">預計到貨日</label>
              <input type="date" value={form.expected_date || ""} onChange={e => setForm(f => ({ ...f, expected_date: e.target.value }))} className={inputCls} /></div>
            <div><label className="block text-base font-semibold text-gray-600 mb-1">狀態</label>
              <select value={form.status} onChange={e => setForm(f => ({ ...f, status: e.target.value }))} className={`${inputCls} bg-white`}>
                {STATUS_OPTS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}</select></div>
            <div><label className="block text-base font-semibold text-gray-600 mb-1">稅別</label>
              <select value={form.tax_type} onChange={e => { const t = e.target.value; setForm(f => ({ ...f, tax_type: t, ...calcTotals(items, t) })) }} className={`${inputCls} bg-white`}>
                {TAX_TYPES.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}</select></div>
          </div>
        </section>
        <section className="bg-white rounded-2xl p-6 shadow-sm">
          <div className="flex items-center justify-between mb-5 pb-3 border-b border-gray-100">
            <h2 className="text-xl font-bold text-gray-700">採購明細</h2>
        ...
        <div className="flex justify-end gap-3 pb-8">
          <Link href="/purchase" className="px-8 py-3 border-2 border-gray-200 text-lg rounded-xl hover:bg-gray-50">取消</Link>
          <button onClick={handleSave} disabled={saving}
